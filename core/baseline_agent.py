"""
Baseline Agent — No Drift Detection or Correction
...
class BaselineAgent:
    """
    Standard ReAct-style agent without drift detection.
    ...
    def __init__(self, embedder, llm=None):
        ...
        self.drift_detector = DriftDetector(embedder, llm=None, threshold=0.5)  # For measurement only
        self._seed_rag()

    def run(self, goal_memory, max_iterations: int = 5) -> dict:
        """
        Run baseline agent and return evaluation metrics.
        """
        ...
        print(f"\n{'─'*60}")
        print("BASELINE AGENT — Standard ReAct (No Drift Detection)"
        print(f"{'─'*60}")
        ...
        for i in range(max_iterations):
            ...
            # Calculate drift score (for measurement only - no correction)
    ...
    def _calculate_adherence(self, goal: str, steps: list) -> float:
        """Simple adherence calculation for baseline."""
