"""
LLM Judge-Only Agent — Drift Detection with LLM Judge Only
...
class LLMJudgeOnlyAgent:
    """
    Agent with LLM judge-based drift detection only.
    Uses LLM alignment rating but no embeddings.
    """
    ...
    def __init__(self, embedder, llm=None):
        ...
        # Drift detector with LLM judge only (no embeddings)
        self.drift_detector = DriftDetector(embedder=None, llm=self.llm, threshold=0.5)
        self.correction_module = CorrectionModule()
        self._seed_rag()

    def run(self, goal_memory, max_iterations: int = 5) -> dict:
        """
        Run LLM judge-only agent and return evaluation metrics.
        """
        ...
        print(f"\n{'─'*60}")
        print("LLM JUDGE-ONLY AGENT — LLM Alignment Rating"
        print(f"{'─'*60}")
        ...
        for i in range(max_iterations):
            ...
            # Check for drift (LLM judge only)
            drift_result = self.drift_detector.check_drift(goal_text, None, step)
            ...
            print(f"LLM judge: {drift_result.get('llm_score', 'N/A')}/5")
